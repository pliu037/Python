...
def r_squared(y, estimated):
    ...
    """
    mean = sum(y)/len(y)
    variance_of_prediction = 0
    variance_of_samples = 0
    for i in range(len(y)):
        variance_of_prediction += (y[i] - estimated[i])**2
        variance_of_samples += (y[i] - mean)**2
    return 1- variance_of_prediction/variance_of_samples
...
def evaluate_models_on_training(x, y, models):
    ...
    r_squareds = []
    for model in models:
        estimated_ys.append(pylab.polyval(model, x))
        r_squareds.append(r_squared(y, estimated_ys[-1]))
        pylab.plot(estimated_ys[-1], label=str(len(model)))
        print(len(model), r_squareds[-1])
    pylab.plot(y, label="actual data")
    pylab.show()
